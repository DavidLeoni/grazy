...
var DebugConstraint = (function () {
    function DebugConstraint() {
    }
    DebugConstraint.create = function (graphicsDevice, technique, mathsDevice, camera) {
        var tech = technique;
        ...
        function drawSingleLineFn(startPoint, endPoint, color) {
            setupGd();
            var writer = gd.beginDraw(gd.PRIMITIVE_LINES, 2, ['FLOAT3', 'FLOAT3'], posColSem);
        ...
        function drawTransform(transform, scale) {
            ...
            var transformWriter = gd.beginDraw(gd.PRIMITIVE_LINES, 6, ['FLOAT3', 'FLOAT3'], posColSem);
        ...
        function drawArcFn(center, normal, axis, radiusA, radiusB, minAngle, maxAngle, color, stepDegrees, curve) {
            ...
            var step = stepDegrees * 0.0174532925;
            ...
            var prev = md.v3Add(md.v3Add(center, md.v3ScalarMul(vx, (radiusA * Math.cos(minAngle)))), md.v3ScalarMul(vy, (radiusB * Math.sin(minAngle))), prev);

            setupGd();

            var writer = gd.beginDraw(gd.PRIMITIVE_LINES, (2 * nSteps) + sectLines, ['FLOAT3', 'FLOAT3'], posColSem);
            if (!curve) {
            ...
            for (var i = 1; i <= nSteps; i += 1) {
                ...
                next = md.v3Add(md.v3Add(center, md.v3ScalarMul(vx, (radiusA * Math.cos(angle)))), md.v3ScalarMul(vy, (radiusB * Math.sin(angle))), next);
        ...
        function pointForAngleFn(angleInRadians, length, swingSpan1, swingSpan2) {
            ...
            var swingLimit = swingSpan1;
            if (Math.abs(xEllipse) > 0.0000001) {
        ...
        function drawConeFn(tr, swingSpan1, swingSpan2, scale) {
            ...
            var writer = gd.beginDraw(gd.PRIMITIVE_LINES, (2 * nSegments) + ((nSegments / (nSegments / 8)) * 2), ['FLOAT3', 'FLOAT3'], posColSem);
        ...
        function drawBoxFn(bbMin, bbMax, tr, color) {
            ...
            var writer = gd.beginDraw(gd.PRIMITIVE_LINES, 24, ['FLOAT3', 'FLOAT3'], posColSem);
        ...
        debug.drawHingeConstraintLimits = function drawHingeConstraintLimitsFn(constraint, scale) {
            ...
            var center = md.m43Pos(trB);
            var normal = basis.slice(6, 9);
            var axis = basis.slice(0, 3);
        ...
        debug.drawP2PConstraintLimits = function drawP2PConstraintLimitsFn(constraint, scale) {
            ...
            pivot = md.v3Add(md.m43TransformVector(constraint.bodyA.transform, pivot, pivot), md.m43Pos(constraint.bodyA.transform), pivot);
            ...
            pivot = md.v3Add(md.m43TransformVector(constraint.bodyB.transform, pivot, pivot), md.m43Pos(constraint.bodyB.transform), pivot);
        ...
        debug.drawConeTwistConstraintLimits = function drawConeTwistConstraintLimitsFn(constraint, scale) {
            ...
            var normal = trA.slice(0, 3);
            var axis1 = trA.slice(3, 6);
        ...
        debug.draw6DOFConstraintLimits = function draw6DOFConstraintLimitsFn(constraint, scale) {
            ...
            var ay = eulerangles[1];
            var az = eulerangles[2];
            var cy = Math.cos(ay);
            var sy = Math.sin(ay);
            var cz = Math.cos(az);
            var sz = Math.sin(az);
            var ref = md.v3Build((cy * cz * axis[0]) + (cy * sz * axis[1]) - (sy * axis[2]), (-sz * axis[0]) + (cz * axis[1]), (cz * sy * axis[0]) + (sz * sy * axis[1]) + (cy * axis[2]), ref);
    ...
    return DebugConstraint;
})();
//# sourceMappingURL=debugphysics.js.map
